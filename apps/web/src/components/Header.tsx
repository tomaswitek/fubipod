...
export async function Header(props: Props) {
  ...
  const contacts = [
    {label: "E", value: global.email, href: `mailto:${global.email}`},
    {label: "T", value: global.phone, href: `tel:${global.phone}`},
  ];
  ...
  return (
    <header className="z-50">
      <div className="bg-white lg:flex p-6 lg:px-8 py-4 justify-between hidden text-gray-600 text-xl">
        <div className="align">
          <LanguageNavigation languages={languages} />
        </div>
        <div className="flex">
          <div className="flex">
            {contacts.map((c) => (
          ...
          <div className="flex mx-2">
            {global.social_links.map((item) => {
              const Icon = SOCIAL_ICONS[item.service];
              return (
                <a
                  key={item.service}
                  href={item.url}
